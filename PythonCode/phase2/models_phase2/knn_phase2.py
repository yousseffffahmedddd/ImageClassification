...
def plot_final_diagnostics(k_results, k_vals, train_accs, val_accs, names, accs, f1s):
    ...
    plt.bar(x - 0.2, accs, 0.4, label='Accuracy', color='#42A5F5')
    plt.bar(x + 0.2, f1s, 0.4, label='Weighted F1', color='#1E88E5')
    plt.xticks(x, names)
...
class KNN_Baseline:
    ...
    def predict(self, X_test):
        dists = np.sum(X_test**2, axis=1, keepdims=True) + np.sum(self.X**2, axis=1) - 2*np.dot(X_test, self.X.T)
        return np.array([np.bincount(self.y[np.argsort(d)[:self.k]]).argmax() for d in dists])

class EnsembleKNN:
    def __init__(self, k=3, n_estimators=5):
        self.k = k
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            idx = np.random.choice(n_samples, int(n_samples * 0.8), replace=True)
            self.models.append((X[idx], y[idx].astype(int)))
