...
y_test_final = (y_test_raw).astype(int)           # until final evaluation
...
def preprocess(model_name, X_tr, X_val, X_te):
    ...
    if model_name == 'KNN':
        ...
        X_tr_sc  = pca.fit_transform(X_tr)
        X_val_sc = pca.transform(X_val)
        X_te_sc  = pca.transform(X_te)
...
def compute_metrics(y_true, y_pred):
    ...
    prec = float(np.mean(precs))   # macro average
    rec  = float(np.mean(recs))
    f1   = float(np.mean(f1s))

    # confusion matrix is now 10×10 — stored separately
...
def plot_confusion_matrix(cm, model_name, condition, save_dir='plots'):
    ...
    n      = cm.shape[0]                      # 10 for multi-class
...
def plot_comparison(all_results, save_dir='plots'):
    ...
    conditions  = ['imbalanced']
    colors      = {'imbalanced': '#E74C3C'}
...
def plot_all_cms(all_results, save_dir='plots'):
    ...
    # Only one condition now (no balanced experiment)
    ...
    for col, name in enumerate(model_names):
        ax = axes[col]
        cm = all_results[name]['imbalanced']['cm']   # 10×10 array
